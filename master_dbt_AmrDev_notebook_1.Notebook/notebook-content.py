...
notebook_files1 = ['model.dbt_AmrDev.my_second_dbt_model', 'test.dbt_AmrDev.not_null_my_first_dbt_model_id.5fb22c2710', 'test.dbt_AmrDev.unique_my_first_dbt_model_id.16e066b321'] # type: ignore
...
df_execution_log = spark.sql(f"SELECT * FROM lh_fa_amr_conformed.execution_log WHERE batch_id = '{pm_batch_id}' AND master_notebook = '{pm_master_notebook}'") # type: ignore
...
if failed_results.count() == 0:
    ...
    df_log.write.format("delta").mode("append").saveAsTable("lh_fa_amr_conformed.execution_log")
else:
